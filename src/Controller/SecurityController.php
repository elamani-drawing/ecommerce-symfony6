...
class SecurityController extends AbstractController
{
    ...
    public function forgottenPassword(Request $request, UsersRepository $usersRepository, TokenGeneratorInterface $tokenGeneratorInterface, EntityManagerInterface $entityManager, SendMailService $mail): Response
    {
        ...
        if($form->isSubmitted() && $form->isValid())
        {
            ...
            if($user)
            {
                //generation d'un token de réinitialisation
                //on pourrait utiliser le jwt mais la on va en essayer un autre
                $token = $tokenGeneratorInterface->generateToken();
                $user->setResetToken($token);
                $entityManager->persist($user);
                $entityManager->flush();

                //on genere un lien de rénitialisation de mdp
    ...
    public function resetPass(string $token, Request $request, UsersRepository $usersRepository, UserPasswordHasherInterface $passwordHasher, EntityManagerInterface $entityManager): Response
    {
        ...
        $this->addFlash('danger', 'Jetons invalide');
        return $this->redirectToRoute('app_login');
    }
